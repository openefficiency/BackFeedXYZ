/*
  # Fix HR Users Insert Error and Complete Schema Setup

  This migration fixes the hr_users insert error by removing the non-existent 'department' column
  and ensures all tables, indexes, RLS policies, and sample data are properly created.

  1. Tables Created/Verified
    - cases (with all required columns and constraints)
    - transcripts (with ElevenLabs voice integration support)
    - hr_interactions (for two-way communication)
    - ai_insights (for AI analysis results)
    - hr_users (without department column)
    - webhook_logs (for ElevenLabs webhook tracking)

  2. Security
    - Enable RLS on all tables
    - Create comprehensive RLS policies for public and authenticated access
    - Ensure proper access control for HR operations

  3. Sample Data
    - 12 realistic test cases covering various HR scenarios
    - Comprehensive transcripts with realistic employee feedback
    - HR interactions showing two-way communication
    - AI insights with risk assessment and next steps
    - 5 HR users for testing authentication
...
-- Create transcripts table with ElevenLabs support
CREATE TABLE IF NOT EXISTS transcripts (
  ...
  created_at timestamptz DEFAULT now(),
  elevenlabs_job_id varchar(255) UNIQUE,
  audio_duration integer DEFAULT 0,
  confidence_score numeric(5,4) DEFAULT 0.0 CHECK (confidence_score >= 0.0 AND confidence_score <= 1.0),
...
CREATE TABLE IF NOT EXISTS ai_insights (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  case_id uuid NOT NULL REFERENCES cases(id) ON DELETE CASCADE,
  insight_type text NOT NULL CHECK (insight_type IN ('risk_assessment', 'next_steps', 'similar_cases', 'sentiment_analysis', 'elevenlabs_conversation', 'elevenlabs_realtime', 'deepgram_realtime', 'deepgram_processing', 'conversation_quality', 'emotional_analysis')),
  content jsonb NOT NULL,
  confidence_score numeric(3,2) DEFAULT 0.0 CHECK (confidence_score >= 0.0 AND confidence_score <= 1.0),
  created_at timestamptz DEFAULT now()
);

-- Create hr_users table (without department column)
...
-- Create webhook_logs table for ElevenLabs integration
CREATE TABLE IF NOT EXISTS webhook_logs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  request_id varchar(255) UNIQUE NOT NULL,
  event_type varchar(100) NOT NULL,
  elevenlabs_job_id varchar(255),
  case_confirmation_code text,
...
CREATE INDEX IF NOT EXISTS idx_transcripts_elevenlabs_job_id ON transcripts(elevenlabs_job_id);
CREATE INDEX IF NOT EXISTS idx_transcripts_processing_status ON transcripts(processing_status);
CREATE INDEX IF NOT EXISTS idx_transcripts_case_id_status ON transcripts(case_id, processing_status);
...
CREATE INDEX IF NOT EXISTS idx_webhook_logs_elevenlabs_job_id ON webhook_logs(elevenlabs_job_id);
CREATE INDEX IF NOT EXISTS idx_webhook_logs_case_id ON webhook_logs(case_id);
...
-- Create function for voice completion trigger
CREATE OR REPLACE FUNCTION update_case_on_voice_completion()
RETURNS TRIGGER AS $$
BEGIN
  -- Update case when voice transcription is completed
...
DROP TRIGGER IF EXISTS trigger_update_case_on_voice_completion ON transcripts;
...
-- Insert HR Users (without department column and with valid roles)
INSERT INTO hr_users (name, email, role) VALUES
  ('Sarah Johnson', 'hr@company.com', 'hr_manager'),
  ('Michael Chen', 'admin@company.com', 'hr_admin'),
  ('Emily Rodriguez', 'specialist@company.com', 'hr_specialist'),
  ('David Kim', 'david.kim@company.com', 'hr_admin'),
  ('Lisa Wang', 'lisa.wang@company.com', 'hr_manager');
